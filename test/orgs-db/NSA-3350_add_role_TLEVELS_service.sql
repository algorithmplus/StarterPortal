    ...
    IF (@serviceId IS NOT NULL)
        ...
        IF (@policyId IS NOT NULL)
            ...
            BEGIN
                ...
                SET @pearsonOrgId = (SELECT id FROM organisation where name = 'PEARSON EDUCATION LIMITED' and Category = '051');
                SET @cityGuildsOrgId = (SELECT id FROM organisation where name = 'CITY AND GUILDS OF LONDON INSTITUTE' and Category = '051');

                --Create Policy Conditions
                INSERT INTO PolicyCondition (Id, PolicyId, Field, Operator, Value, CreatedAt, UpdatedAt)
                VALUES  (NEWID(), @policyId, 'organisation.id', 'is', @pearsonOrgId, GETDATE(), GETDATE()),
