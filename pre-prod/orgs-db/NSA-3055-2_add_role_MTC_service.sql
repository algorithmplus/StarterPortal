...
BEGIN TRY
    ...
    IF (@serviceId IS NOT NULL)
        ...
        -- Step 2) get role Id based on name, if more than one found it will fail and go to catch block
        SET @roleId = (SELECT id FROM Role WHERE name = @roleName);
        IF (@roleId IS NOT NULL)
            BEGIN

        -- Step 3) get policy Id based on name, if more than one found it will fail and go to catch block
